...
- **Folder-based installs are not indexed.** Some systems (notably PS3/RPCS3-style decrypted game directories) are distributed as a folder tree rather than a single file. The scanner only ever indexes individual files, so these folders are currently invisible to ROM Manager entirely — not scanned, not exported, not verified. Supporting them would mean treating a directory as one logical game, which touches scanning, hashing, preferred-copy, and export logic throughout the app; not yet implemented.
